...
const projects = [
  { id: '01', title: 'Nawa Records', type: 'Identity / Art Direction', art: 'nawa' },
  { id: '02', title: 'Object 17', type: 'Editorial / Print', art: 'object' },
  { id: '03', title: 'Kora', type: 'Packaging / Identity', art: 'kora' },
  { id: '04', title: 'Field Notes', type: 'Campaign / Typography', art: 'field' },
  { id: '05', title: 'Miro House', type: 'Hospitality / Digital', art: 'miro' },
  { id: '06', title: 'Index 26', type: 'Publication / System', art: 'index' },
  { id: '07', title: 'Rê', type: 'Culture / Poster Series', art: 're' },
  { id: '08', title: 'North Studio', type: 'Strategy / Identity', art: 'north' },
  { id: '09', title: 'Mono / 09', type: 'Typography / Poster', art: 'mono' },
  { id: '10', title: 'Arc House', type: 'Architecture / Identity', art: 'arc' },
  { id: '11', title: 'Signal', type: 'Campaign / Motion', art: 'signal' },
  { id: '12', title: 'Atelier 41', type: 'Fashion / Art Direction', art: 'atelier' },
  { id: '13', title: 'Fold', type: 'Packaging / Print', art: 'fold' },
  { id: '14', title: 'Echo Journal', type: 'Editorial / Publication', art: 'echo' },
  { id: '15', title: 'Terra', type: 'Culture / Identity', art: 'terra' },
  { id: '16', title: 'Volume', type: 'Music / Campaign', art: 'volume' },
];
...
const wireInputCards = [
  { id: 'ideas', label: 'Ideas', detail: 'The first clear thought.', Icon: Lightbulb },
  { id: 'inspiration', label: 'Inspiration', detail: 'References with a reason.', Icon: FileText },
  { id: 'conversation', label: 'Conversations', detail: 'What people actually mean.', Icon: ChatCircleDots },
  { id: 'culture', label: 'Culture', detail: 'Context, place and behavior.', Icon: UsersThree },
];

const wireOutputCards = [
  { id: 'routine', art: 'routine', title: 'The New Routine', kind: 'Brand Identity', detail: 'Tactile form, packaging and image language.' },
  { id: 'sora', art: 'sora', title: 'Sora', kind: 'Visual Campaign', detail: 'A monochrome campaign built around atmosphere.' },
  { id: 'feeling', art: 'feeling', title: 'Form Follows Feeling', kind: 'Editorial Design', detail: 'A soft editorial system driven by material.' },
  { id: 'closer', art: 'closer', title: 'People Closer', kind: 'Digital Experience', detail: 'A quiet digital identity with strong typography.' },
];
...
function WireSystem() {
  ...
  return (
    <div className="wire-stage" aria-label="Ideas flow into Miran Azad's design process and become finished graphic work">
      <div className="wire-main" ref={wireMainRef}>
        ...
        <div className="wire-inputs">
          {wireInputCards.map(({ id, label, Icon }) => (
            <div
              className="idea-node"
              key={id}
        ...
        <div className="processor-box" ref={processorRef}>
          <strong>Miran Azad</strong>
          <span>GRAPHIC DESIGNER</span>
        </div>

        <div className="wire-output">
          {wireOutputCards.map((card) => (
            <article
              ...
              onLostPointerCapture={endFreeDrag}
              onPointerLeave={clearForcedFront}
              aria-label={`${card.title}. Drag freely to reposition.`}
            >
              <span className="wire-drag-handle" aria-hidden="true" />
...
function App() {
  ...
  return (
    <div ref={pageRef} className="min-h-[100dvh] overflow-x-clip bg-[#f7f7f4] text-[#181817]">
      <header className="site-nav fixed inset-x-0 top-0 z-40 mx-auto flex h-[76px] max-w-[1380px] items-center justify-between px-5 md:px-8 xl:px-12">
        <a href="#top" className="nav-mark" aria-label="Miran Azad home">MA<span>®</span></a>
        <nav className="hidden items-center gap-9 text-[12px] font-semibold uppercase tracking-[0.17em] md:flex" aria-label="Primary navigation">
          <a href="#work">Work</a>
        ...
        <button className="mobile-menu-btn md:hidden" onClick={() => setMenuOpen((v) => !v)} aria-label="Toggle navigation" aria-expanded={menuOpen}>
          {menuOpen ? <X size={22} /> : <List size={24} />}
        </button>
      </header>

      <div className={`mobile-menu ${menuOpen ? 'is-open' : ''}`} aria-hidden={!menuOpen}>
        <a href="#work" onClick={closeMenu} tabIndex={menuOpen ? 0 : -1}>Work</a>
        <a href="#about" onClick={closeMenu} tabIndex={menuOpen ? 0 : -1}>Profile</a>
        <a href="#services" onClick={closeMenu} tabIndex={menuOpen ? 0 : -1}>Services</a>
        <a href="#contact" onClick={closeMenu} tabIndex={menuOpen ? 0 : -1}>Start a project <ArrowUpRight size={24} /></a>
      </div>
        ...
        <section className="hero-section relative mx-auto grid max-w-[1380px] grid-cols-1 items-center gap-12 px-5 pb-10 pt-28 md:px-8 md:pt-32 xl:grid-cols-[1.08fr_.92fr] xl:gap-16 xl:px-12">
          <div className="hero-copy pb-2 xl:pb-4">
            <div className="hero-reveal mb-12 hidden items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#66665f] md:flex">
              <span className="inline-block size-2 rounded-full bg-[#97a64b]" /> Independent graphic designer / Erbil
            </div>
            <h1 className="hero-reveal hero-title">
              <span className="hero-title-line hero-title-primary">
                <ElasticWord text="Clear" />
                <ElasticWord text="Ideas." className="hero-title-accent" />
              </span>
              <span className="hero-title-line hero-title-final">
                <ElasticWord text="Sharp Form." />
              </span>
            </h1>
            <p className="hero-reveal mt-7 max-w-[34rem] text-[17px] leading-[1.45] text-[#55554f] md:mt-8 md:text-[20px]">
              Identity, art direction and graphic systems for culture, hospitality and ambitious independent brands.
            </p>
            <div className="hero-reveal mt-10 flex flex-wrap items-center gap-4 md:mt-14">
              <a className="primary-cta" href="#work">View selected work <ArrowDown size={18} weight="bold" /></a>
            </div>
          </div>
          ...
          >
            ...
            <div className="hero-specimen-topline">
              ...
              <span>GRAPHIC DESIGN / ERBIL</span>
            </div>
            ...
            <div className="hero-sheet hero-sheet-front">
              ...
              <div className="hero-kurdish-sun" aria-label="Kurdish sun">
              ...
              <div className="hero-sheet-foot">
                <strong>Miran Azad</strong>
                <span>Identity / Art direction / Digital</span>
              </div>
            </div>
        ...
        <section id="system" className="border-t border-[#d8d8d1] px-5 py-12 md:px-8 md:py-16 xl:px-12">
          <div className="mx-auto max-w-[1380px]">
            <div className="section-reveal mb-9 md:mb-10">
              <h2 className="section-title text-[clamp(3rem,4.5vw,5.1rem)] font-[500] leading-[0.88] tracking-[-0.06em]">From Idea to Impact</h2>
              <p className="mt-5 max-w-[430px] text-[16px] leading-[1.5] text-[#66665f]">Ideas, context and conversation become one visual system.</p>
            </div>
            <WireSystem />
          </div>
        </section>
        ...
        <section id="about" className="border-t border-[#d8d8d1] px-5 py-20 md:px-8 md:py-24 xl:px-12">
          <div className="mx-auto grid max-w-[1380px] gap-9 lg:grid-cols-[.9fr_1.1fr] lg:items-start lg:gap-12 xl:grid-cols-[.86fr_1.14fr] xl:gap-16">
            <div className="profile-visual section-reveal relative aspect-square overflow-hidden border border-[#d7d7d0] bg-[#efefec]">
              <img className="profile-portrait absolute inset-0 h-full w-full object-cover" src={`${import.meta.env.BASE_URL}il_fullxfull.8346800249_jsz5.avif`} alt="Portrait of Miran Azad" />
              <div className="absolute inset-x-0 bottom-0 grid grid-cols-2 border-t border-[#d1d1cb] bg-[#f7f7f4]/90 text-[#181817] backdrop-blur-md">
                <div className="border-r border-[#d1d1cb] px-5 py-4"><strong className="block text-[18px] font-[500]">Erbil / Iraq</strong></div>
                <div className="px-5 py-4"><strong className="block text-[18px] font-[500]">Worldwide</strong></div>
              </div>
            </div>
            <div className="section-reveal lg:pt-1">
              <h2 className="section-title max-w-[820px] text-[clamp(4.4rem,7.4vw,8rem)] font-[500] uppercase leading-[0.78] tracking-[-0.067em]">Miran Azad.</h2>
              <div className="mt-7 grid gap-7 border-t border-[#cecec7] pt-6 md:grid-cols-[1.12fr_.88fr] lg:mt-8 lg:gap-10">
                <p className="max-w-[650px] text-[20px] leading-[1.46] text-[#3f3f3b] md:text-[22px] lg:text-[23px]">I build identities and graphic systems that feel distinct without needing to shout. Find the strongest idea, remove what weakens it, and make every detail support the same point.</p>
                <div className="text-[15px] font-medium leading-[1.65] text-[#5f5f59] md:text-[16px]">
                  <p>Independent since 2019.<br />Brand systems / Art direction / Editorial / Digital.</p>
        ...
        <section id="work" className="border-t border-[#d8d8d1] px-5 py-28 md:px-8 md:py-36 xl:px-12">
          <div className="mx-auto max-w-[1380px]">
            <div className="section-reveal mb-16 flex flex-col gap-8 md:mb-24 md:flex-row md:items-end md:justify-between">
              <div>
                <h2 className="section-title text-[clamp(3rem,5.5vw,6rem)] font-[500] uppercase leading-[0.84] tracking-[-0.062em]">Selected Work.</h2>
              </div>
              <p className="max-w-[420px] text-[15px] leading-[1.55] text-[#66665f]">Identity, campaigns, print and digital systems across different scales.</p>
            </div>
            ...
            <div className="projects-grid grid grid-cols-1 gap-x-4 gap-y-12 sm:grid-cols-2 lg:grid-cols-4 lg:gap-y-14">
              {projects.map((project) => (
                <article key={project.id} className="project-tile group">
                  <a href="#contact" className="block" aria-label={`${project.title}, ${project.type}`}>
                    ...
                    <div className="mt-4 flex items-start justify-between gap-4 border-t border-[#d0d0c9] pt-3">
                      <div><h3 className="text-[17px] font-[520] tracking-[-0.02em]">{project.title}</h3><p className="mt-1 text-[11px] uppercase tracking-[0.14em] text-[#66665f]">{project.type}</p></div>
                    </div>
                  </a>
                </article>
              ))}
          ...
          <div className="marquee-track flex min-w-max items-center gap-12 whitespace-nowrap text-[clamp(2.8rem,6vw,6.5rem)] font-[500] uppercase leading-none tracking-[-0.055em] text-[#242421]">
            {[0, 1].map((n) => (
              <div key={n} className="flex items-center gap-12" aria-hidden={n === 1}>
                <span>Identity</span><i /> <span>Typography</span><i /> <span>Direction</span><i /> <span>Editorial</span><i /> <span>Digital</span><i />
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="services" className="px-5 py-28 md:px-8 md:py-36 xl:px-12">
          <div className="mx-auto max-w-[1380px]">
            <div className="section-reveal mb-14 grid gap-8 lg:grid-cols-2 lg:items-end">
              <div><h2 className="section-title text-[clamp(3rem,5.2vw,5.8rem)] font-[500] uppercase leading-[0.84] tracking-[-0.06em]">What I Do.</h2></div>
              <p className="max-w-[470px] justify-self-start text-[16px] leading-[1.55] text-[#66665f] lg:justify-self-end">Identity, art direction, editorial systems and digital design.</p>
            </div>
            <div className="section-reveal border-t border-[#bfbfb8]">
              {services.map(({ title, copy, Icon }) => (
                <article key={title} className="service-row group grid gap-6 border-b border-[#cfcfc8] py-7 md:grid-cols-[1fr_1fr_120px] md:items-center md:py-9">
                  <h3 className="text-[clamp(1.8rem,3vw,3.2rem)] font-[480] tracking-[-0.045em]">{title}</h3>
                  <p className="max-w-[490px] text-[14px] leading-[1.55] text-[#696962]">{copy}</p>
        ...
        <section className="border-y border-[#d8d8d1] bg-[#ecece6] px-5 py-28 md:px-8 md:py-36 xl:px-12">
          <div className="mx-auto max-w-[1380px]">
            <div className="section-reveal grid gap-14 lg:grid-cols-[.72fr_1.28fr] lg:gap-20">
              <div>
                <p className="max-w-[330px] text-[17px] leading-[1.55] text-[#5f5f59]">Small teams, direct conversations, fewer handoffs. The person you brief is the person making the work.</p>
              </div>
              <div className="process-list">
                {[['Find the real problem','Research, references, constraints and a sharp creative premise.'],['Build the language','Type, image, layout, color and rules become one connected system.'],['Test it in context','The system is pushed across the real touchpoints before anything is called finished.'],['Make it usable','Clear files, templates and guidance so the work survives after handoff.']].map(([title,copy]) => (
                  <div className="process-row" key={title}>
                    <h3>{title}</h3><p>{copy}</p><ArrowRight size={26} weight="light" />
                  </div>
                ))}
              </div>
            </div>
        ...
        <section id="contact" className="px-5 pb-8 pt-28 md:px-8 md:pt-36 xl:px-12">
          <div className="section-reveal mx-auto max-w-[1380px] border-t border-[#bdbdb5] pt-8">
            <div className="grid gap-10 lg:grid-cols-[1.4fr_.6fr] lg:items-end">
              <div>
                <h2 className="section-title text-[clamp(3.8rem,8.8vw,9rem)] font-[510] uppercase leading-[0.76] tracking-[-0.072em]">Have a Project?</h2>
              </div>
              <div className="pb-2 lg:pb-5">
                <p className="max-w-[390px] text-[17px] leading-[1.5] text-[#595953]">Send the project, timing and what is not working yet. I usually reply within two working days.</p>
            ...
            <footer className="mt-24 flex flex-col gap-5 border-t border-[#d1d1ca] py-6 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#74746d] md:mt-32 md:flex-row md:items-center md:justify-between">
              <span>© 2026 Miran Azad</span>
              <div className="flex gap-6"><a href="#top">Top</a><a href="#work">Work</a><a href="mailto:hello@miranazad.com">Email</a></div>
            </footer>
          </div>
        </section>
      </main>
